 ...
 */
abstract class Settings_Section_Text_Element_Abstract extends Settings_Section_Element_Abstract {
	...
	public function print_field_callback( $arguments ): void {
		...
		if ( isset( $arguments['placeholder'] ) ) {
			...
			printf( '<input name="%1$s" id="%1$s" type="text" value="%2$s" />', esc_attr( $this->get_id() ), esc_attr( $value ) );
		}
